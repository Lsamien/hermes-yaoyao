...
test('created roles and editable teams share a durable chat list without plugin calls', async ({
  ...
  const collapse = rail.getByRole('button', { name: '折叠侧边栏', exact: true })
  expect((await plus.boundingBox())!.x).toBeGreaterThan((await collapse.boundingBox())!.x)
  await collapse.click()
  await expect(rail).toHaveClass(/desktop-sidebar--collapsed/)
  const logoBox = (await rail.locator('.sidebar-brand').boundingBox())!
  const plusBox = (await plus.boundingBox())!
  expect(plusBox.y).toBeGreaterThanOrEqual(logoBox.y + logoBox.height)
  await plus.click()
  await expect(page.getByRole('menu', { name: '新建聊天' }).getByRole('menuitem')).toHaveCount(2)
  await page.screenshot({ path: testInfo.outputPath('collapsed-create-menu.png'), fullPage: true })
  await page.keyboard.press('Escape')
  await rail.getByRole('button', { name: '搜索', exact: true }).click()
  await expect(page.getByRole('dialog', { name: '搜索聊天' })).toBeVisible()
  await expect(rail).toHaveClass(/desktop-sidebar--collapsed/)
  await page.keyboard.press('Escape')
  await expect(rail.getByRole('button', { name: '搜索', exact: true })).toBeFocused()
  await rail.getByRole('button', { name: '展开侧边栏', exact: true }).click()
  ...
  const mobile = page.locator('.mobile-drawer')
  await expect(mobile.locator('.sidebar-feature-nav')).toHaveCount(0)
  ...
  await page.keyboard.press('Escape')
  await mobile.getByRole('button', { name: '搜索', exact: true }).click()
  await expect(page.getByRole('dialog', { name: '搜索聊天' })).toBeVisible()
  await expect(page.getByRole('tab', { name: '未归档', exact: true })).toBeVisible()
  await page.screenshot({ path: testInfo.outputPath('mobile-chat-search.png'), fullPage: true })
