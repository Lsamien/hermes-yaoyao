...
test('selects existing group topics and creates a new protocol v4 topic on first send', async ({ page }) => {
  ...
  await expect(backToRooms).toContainText('返回房间列表')
  ...
  await expect(sidebar.getByRole('button', { name: '新建群聊' })).toBeHidden()
  ...
  await expect(sidebar.getByRole('button', { name: '新建群聊' })).toBeVisible()
