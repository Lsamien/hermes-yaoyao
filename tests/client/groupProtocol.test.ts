...
describe('group protocol continuity', () => {
  ...
  it('normalizes v5 host fields while keeping v4 agents non-host by default', () => {
    ...
    expect(isSupportedGroupProtocolVersion(12)).toBe(true)
    expect(isSupportedGroupProtocolVersion(13)).toBe(false)
    expect(SUPPORTED_GROUP_PROTOCOL_VERSION_LABEL).toBe('v2–v12')
  })
})
