...
describe('authenticated read cache routes', () => {
  it('hits cached reads, honors fresh reads and never bypasses logout authentication', async () => {
    ...
    const config: ServerConfig = { host: '127.0.0.1', port: 8800, upstream: new URL('http://127.0.0.1:19119'),
    ...
    const start = await agent.get('/api/app/bootstrap').set('Host', '127.0.0.1:8800').expect(200)
    const login = await agent.post('/api/app/login').set('Host', '127.0.0.1:8800').set('Origin', origin)
      .set('X-CSRF-Token', start.body.csrfToken).send({ username: 'admin', password: 'admin' }).expect(200)
    ...
    const first = await agent.get('/api/sessions').set('Host', '127.0.0.1:8800').expect(200)
    const statusAfterColdRead = statusReads
    const second = await agent.get('/api/sessions').set('Host', '127.0.0.1:8800').expect(200)
    ...
    const fresh = await agent.get('/api/sessions').set('Host', '127.0.0.1:8800').set('X-Yaoyao-Cache', 'bypass').expect(200)
