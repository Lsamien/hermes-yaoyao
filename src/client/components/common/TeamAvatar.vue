...
export type TeamAvatarMember = {
  ...
  activityKey?: number
  avatar?: string
  state?: 'idle' | 'working' | 'waiting' | 'loading' | 'success' | 'failure' | 'notifying'
}
...
const clusterMembers = computed(() => {
  const entries = props.members.slice(0, 3).map(member => ({
    ...
    state: member.state || 'idle', activityKey: member.activityKey,
  }))
  return entries
})
</script>
...
<template>
  ...
  >
    ...
    <AppIcon v-else-if="!clusterMembers.length" name="groups" :size="size" />
    <span v-else class="team-avatar__cluster" aria-hidden="true">
      <AgentAvatar
        ...
        :name="member.name"
        :avatar="member.avatar"
        :state="member.state" :activity-key="member.activityKey"
...
.team-avatar__member { position: absolute; }
