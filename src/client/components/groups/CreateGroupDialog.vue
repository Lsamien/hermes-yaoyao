...
const props = withDefaults(defineProps<{ open: boolean; profiles: string[]; busy?: boolean }>(), { busy: false })
const emit = defineEmits<{ close: []; create: [payload: { name: string; profiles: string[]; autoReply: boolean; replyRounds: number }] }>()

const name = ref('')
const selected = ref<string[]>([])
const autoReply = ref(true)
const replyRounds = ref(3)
const valid = computed(() => name.value.trim().length > 0 && selected.value.length >= 1 && selected.value.length <= 8)
...
function create() {
  ...
  emit('create', { name: name.value.trim(), profiles: selected.value, autoReply: autoReply.value, replyRounds: replyRounds.value })
}
</script>
      ...
      <div v-if="open" class="dialog-layer" role="presentation" @mousedown.self="emit('close')">
        <section class="create-dialog" role="dialog" aria-modal="true" aria-labelledby="create-group-title">
          ...
          <div class="dialog-settings">
            <label><input v-model="autoReply" type="checkbox" />启用自动回复</label>
