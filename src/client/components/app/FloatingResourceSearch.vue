...
const results = computed(() => {
  ...
  if (!needle) return props.items.slice(0, 12)
    ...
    <div v-if="open" class="floating-resource-search__backdrop" @click.self="close">
      <section class="floating-resource-search" role="dialog" aria-modal="true" :aria-label="label">
        ...
        <div class="floating-resource-search__results" role="listbox" aria-label="搜索结果">
          <button v-for="item in results" :key="item.id" type="button" role="option" @click="choose(item.id)">
            <span class="floating-resource-search__copy"><strong>{{ item.title }}</strong><small v-if="item.subtitle">{{ item.subtitle }}</small></span>
