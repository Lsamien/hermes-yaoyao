...
<template>
  <main class="login-view">
    ...
    <section class="login-panel" aria-labelledby="login-title">
      ...
      <p class="login-note"><span class="status-dot" />凭据只发送到当前 8800 服务，不会写入浏览器存储。</p>
    </section>
  </main>
</template>
...
.login-panel { position: relative; z-index: 2; width: min(368px, 100%); animation: login-enter 360ms var(--ease-out) both; }.login-panel :deep(.brand-mark) { display: flex; flex-direction: column; align-items: flex-start; gap: 16px; }.login-panel :deep(.brand-mark__name) { font-size: 25px; }
.login-copy { margin: 38px 0 22px; }.login-copy h1 { margin: 0; font-size: 18px; font-weight: 630; letter-spacing: -.03em; }.login-copy p { margin: 6px 0 0; color: var(--text-muted); font-size: 11px; }
...
.login-note { display: flex; align-items: center; justify-content: center; gap: 6px; margin: 20px 0 0; color: var(--text-muted); font-size: 9px; line-height: 1.5; text-align: center; }.login-note .status-dot { width: 5px; height: 5px; background: var(--success); }
@keyframes login-enter { from { opacity: 0; transform: translateY(10px); } }
@media (max-width: 480px) { .login-view { place-items: start center; padding: max(74px, calc(env(safe-area-inset-top) + 58px)) 22px 24px; overflow-y: auto; }.login-panel :deep(.brand-mark__plate) { width: 64px !important; height: 64px !important; }.login-copy { margin-top: 30px; } }
</style>
