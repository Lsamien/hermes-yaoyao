...
async function login() {
  ...
  if (auth.setupRequired && password.value !== passwordConfirmation.value) {
  ...
  try {
    ...
    if (auth.setupRequired) await auth.setup(input)
  ...
  <main class="login-view">
    ...
    <section class="login-panel" aria-labelledby="login-title">
      ...
      <div class="login-copy">
        <h1 id="login-title">{{ auth.setupRequired ? '创建管理员' : '登录夭夭' }}</h1>
      ...
      <form @submit.prevent="login">
        ...
        <label v-if="auth.setupRequired"><span>确认密码</span><input v-model="passwordConfirmation" name="password-confirmation" type="password" autocomplete="new-password" minlength="8" required placeholder="再次输入密码" /></label>
        <p v-if="localError || auth.error" class="login-error" role="alert"><AppIcon name="alert" :size="14" />{{ localError || auth.error }}</p>
        <button class="login-submit solid-button" type="submit" :disabled="submitting || !username.trim() || password.length < (auth.setupRequired ? 8 : 1) || (auth.setupRequired && password !== passwordConfirmation)">
          <span>{{ submitting ? '正在提交…' : auth.setupRequired ? '创建并登录' : '登录' }}</span><AppIcon v-if="!submitting" name="arrow-up" :size="15" />
        </button>
      </form>
    </section>
  </main>
